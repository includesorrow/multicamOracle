...
where e.departments_id = d.department_id; -- employees에 있는 departments_id 이렇게연결됨
...
from emp e, dept d
where e.deptno(+) = d.deptno --(+) : outer join : null값 출력
--될 수 있으면 from에 적은 순서대로 조건을 순서대로 맞추는게 좋다
order by ename;
